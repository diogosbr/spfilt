#' Eugenia aurata occurrence data.
#' @description Distribution data for Eugenia aurata. Downloaded from Global Biodiversity Information Facility(GBIF).
#' @references \url{http://www.gbif.org}
"Eugenia_aurata"

#' Leucena leucocephala occurrence data.
#' @description Distribution data for Leucena leucocephala. Downloaded from Global Biodiversity Information Facility(GBIF).
#' @references \url{http://www.gbif.org}
"Leucaena_leucocephala"

#' World map.
#' @description shape.
#' @references \url{http://www.gbif.org}
"world"

#' Rasetr South America.
#' @description raster.
#' @references \url{http://www.gbif.org}
"example"